...
---
Import the **web3** and **@klaytn/web3js-ext** packages to add klaytn features on web3
...
```
---
Setup the provider with the specified Klaytn **Baobab** testnet URL. A provider in web3 is a read-only abstraction to access the blockchain data.
...
```
---
Create a **sender wallet** with the private key using `web3.eth.accounts.privateKeyToAccount`

```js FeeDelegatedSmartContractExecution.js focus=10

```
---
Create a **fee payer wallet** with the private key using `web3.eth.accounts.privateKeyToAccount`
...
```
---
Create a contract instance using `web3.eth.Contract(contractAbi, contractAddr)`
