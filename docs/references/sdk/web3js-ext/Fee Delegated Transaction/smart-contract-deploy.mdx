# Smart Contract Deploy
**TxTypeSmartContractDeploy** deploys a smart contract to the given address. The following changes will be made by this transaction type.

<CH.Spotlight>
<CH.Code>

```js FeeDelegatedSmartContractDeployExample.js
const { Wallet, TxType } = require("@klaytn/ethers-ext");
const ethers = require("ethers");

const senderAddr = "0xa2a8854b1802d8cd5de631e690817c253d6a9153";
const senderPriv = "0x0e4ca6d38096ad99324de0dde108587e5d7c600165ae4cd6c2462c597458c2b8";
const feePayerAddr = "0xcb0eb737dfda52756495a5e08a9b37aab3b271da";
const feePayerPriv = "0x9435261ed483b6efa3886d6ad9f64c12078a0e28d8d80715c773e16fc000cff4";

async function main() {
  const provider = new ethers.providers.JsonRpcProvider("https://public-en-baobab.klaytn.net");
  const senderWallet = new Wallet(senderPriv, provider);
  const feePayerWallet = new Wallet(feePayerPriv, provider);

  let tx = {
    type: TxType.FeeDelegatedSmartContractDeployWithRatio,
    to:    "0x0000000000000000000000000000000000000000",
    value: 0,
    from: senderAddr,
    input: "",
    humanReadable: false,
    feeRatio: 30,
    codeFormat: 0x00,
  };

  tx = await senderWallet.populateTransaction(tx);
  console.log(tx);

  const senderTxHashRLP = await senderWallet.signTransaction(tx);
  console.log("senderTxHashRLP", senderTxHashRLP);

  const sentTx = await feePayerWallet.sendTransactionAsFeePayer(senderTxHashRLP);
  console.log("sentTx", sentTx);

  const rc = await sentTx.wait();
  console.log("receipt", rc);
}

main();
```
---
```zsh output
❯ node FeeDelegatedSmartContractDeployExample.js
senderTxHashRLP 
signedTx 0x57cf3bfddf3e72effc39e80b9cb6995273822e51d50f55da93a14574b5dae9d1
receipt {
  blockHash: '0x84f6aefd1ee6425e03cd8571fbfb653cf5971c729e7b3864456bb36bfd212622',
  blockNumber: 148744885n,
  contractAddress: '0x661c557c1b7c2b1fbd6c4793d210cd1e421064e9',
  cumulativeGasUsed: 333958n,
  effectiveGasPrice: 25000000000n,
  from: '0xa2a8854b1802d8cd5de631e690817c253d6a9153',
  gasUsed: 140317n,
  logs: [],
  logsBloom: '',
  status: 1n,
  transactionHash: '0x57cf3bfddf3e72effc39e80b9cb6995273822e51d50f55da93a14574b5dae9d1',
  transactionIndex: 1n,
  type: 0n
}

```
</CH.Code>

---
Import necessary classes from Klaytn ethers extension and ethers library


```js FeeDelegatedSmartContractDeployExample.js focus=1:2

```
---

Define sender and fee payer addresses and private keys

```js FeeDelegatedSmartContractDeployExample.js focus=4:7

```
---
Create a provider connected to the Klaytn Baobab testnet


```js FeeDelegatedSmartContractDeployExample.js focus=10

```
---

Create sender and fee payer wallets using their private keys and the provider

```js FeeDelegatedSmartContractDeployExample.js focus=11:12

```
---

Define a transaction object
```js FeeDelegatedSmartContractDeployExample.js focus=14:23

```

---
The contract code to deploy

```js FeeDelegatedSmartContractDeployExample.js focus=19

```

---

Define [fee ratio](https://archive-docs.klaytn.foundation/content/klaytn/design/transactions/partial-fee-delegation) paid by sender and fee payer
```js FeeDelegatedSmartContractDeployExample.js focus=21

```
---
Populate the transaction with additional data (e.g., gas, nonce) using sender's wallet


```js FeeDelegatedSmartContractDeployExample.js focus=25:26

```
---
Sign the transaction with the sender's private key


```js FeeDelegatedSmartContractDeployExample.js focus=28:29

```
---
Send the transaction, where the fee is paid by the fee payer wallet


```js FeeDelegatedSmartContractDeployExample.js focus=31:32

```
---
Wait for the transaction receipt


```js FeeDelegatedSmartContractDeployExample.js focus=34:35

```


</CH.Spotlight>
