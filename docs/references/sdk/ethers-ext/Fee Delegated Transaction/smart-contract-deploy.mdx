...
---
Send the fee-delegated transaction using the **fee payer**'s wallet. Other fields that are not declared are filled automatically in **sendTransaction** function. It will return the tx hash, which will be used to wait the receipt
<CH.Code>
