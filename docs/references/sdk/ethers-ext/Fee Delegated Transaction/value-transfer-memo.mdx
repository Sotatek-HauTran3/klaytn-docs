...
Send the tx to the blockchain as **the fee payer**. Other fields that are not declared are filled automatically in **sendTransactionAsFeePayer** function. It will return the tx hash, which will be used to wait the receipt
