...
---
In this case, the account key to update is the **role-based** key and the possible key types of each role are the `roleTransactionKey`, `roleAccountUpdateKey` or `roleFeePayerKey`.
